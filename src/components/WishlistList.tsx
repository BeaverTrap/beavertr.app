...
export default function WishlistList({ wishlistId, isOwner = false }: WishlistListProps) {
  ...
  if (loading) {
    return <div className="text-base-content/70">Loading...</div>;
  }

  if (items.length === 0) {
    return <div className="text-base-content/70">No items yet. Add some!</div>;
  }
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => {
        ...
        <div
          key={item.id}
          className={`p-4 rounded-lg bg-base-200 border transition-colors overflow-hidden ${
            item.isPurchased
        ...
        >
          ...
          {item.image && (
            <a
              ...
              className="block relative w-full h-48 mb-4 rounded-lg overflow-hidden bg-base-300 hover:opacity-90 transition-opacity cursor-pointer"
            >
              <img
                src={item.image}
                alt={item.title}
          ...
          <div className="mb-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block px-2 py-1 text-xs rounded bg-base-300 text-base-content border border-base-300 hover:bg-base-content hover:text-base-100 transition-colors"
          ...
          <a
            ...
            className="block"
          >
            <h3 className="text-lg font-semibold text-base-content mb-2 line-clamp-2 hover:text-primary transition-colors cursor-pointer">
              {item.title}
            </h3>
          </a>
          ...
          <div className="text-xl font-bold text-green-400 mb-2">
            {refreshing.has(item.id) ? (
              <span className="text-base-content/50 text-sm">Updating...</span>
            ) : item.price ? (
              (() => {
              ...
              <span className="text-base-content/50 text-sm">No price</span>
            )}
          </div>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {!item.proofVerified && !item.proofRejected && item.purchaseProof && (isOwner || isModerator) && (
                  <div className="flex gap-2">
                    ...
                    {isModerator && !isOwner && (
                      <span className="text-xs text-base-content/70 px-2 py-1">(Moderator)</span>
                    )}
                  </div>
                )}
              </div>
          ...
          {(item.size || item.quantity || item.notes) && (
            <div className="text-sm text-base-content/70 mb-3 space-y-1">
              {item.size && (
                <div>
                  <span className="text-base-content/50">Size:</span> <span className="text-base-content">{item.size}</span>
                </div>
              )}
              {item.quantity && (
                <div>
                  <span className="text-base-content/50">Quantity:</span> <span className="text-base-content">{item.quantity}</span>
                </div>
              )}
              {item.notes && (
                <div className="italic text-base-content/80">
                  <span className="text-base-content/50">Note:</span> {item.notes}
                </div>
              )}
            </div>
          )}
          ...
          {item.affiliateUrl && (
            <div className="text-xs text-base-content/50 mb-3 p-2 rounded bg-base-300 border border-base-300">
                  ...
                  {item.isClaimed && item.claimedBy === session.user?.id ? (
                    <>
                      ...
                      <button
                        ...
                        className="text-xs px-2 py-1 rounded bg-base-300 hover:bg-base-content hover:text-base-100 text-base-content whitespace-nowrap"
                      >
                        Unclaim
                      </button>
                    </>
